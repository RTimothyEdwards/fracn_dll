...
module dll_controller(reset, clock, osc, div, trim);
    ...
    reg [7:0] accum;
    reg [8:0] tval;	// Fractional part must be > number of frac. bits in div
    wire [4:0] tint;	// Integer part of tval

    assign tint = tval[8:4];
    ...
    always @(posedge clock or posedge reset) begin
        if (reset == 1'b1) begin
            tval <= 9'd0;	// Note:  trim[0] must be zero for startup to work.
            oscbuf <= 3'd0;
	    accum  <= 8'd0;
            count0 <= 5'd0;
            count1 <= 5'd0;
		...
		accum <= accum + count0 - count8;
                ...
                if ({accum, 1'b0} > div) begin
		    if (tval < 511) begin
            	        tval <= tval + 1;
		    end
                end else if ({accum, 1'b0} < div) begin
		    if (tval > 0) begin
            	        tval <= tval - 1;
		    end
                end
